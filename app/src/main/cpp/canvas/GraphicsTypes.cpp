...
namespace Engine {
    ...
    static const unsigned char ColorHashesToIndex[544] = {
    ...
    static inline SkColor toSkColor(const Color& color) {
        return SkColorSetARGB(color.rgba.a, color.rgba.r, color.rgba.g, color.rgba.b);
    }

    static Color HSLAtoColor(float h, float s, float l, float a) {
        h = fmod(h, 1); // wrap around
    ...
    SkColor to_color(const std::string& colorStr) {
        const char* color = colorStr.c_str();
        ...
        if (color[0] == '#' && len == 4) {
            ...
            int component = 0;
            for (int i = 4; i < len - 1 && component < 4; i++) {
                if (component == 3) {
                    ...
                    for (int j = 0; i + j < len - 1 && j < 7; j++) {
                        alpha[j] = color[i + j];
                    }

                    float value = atof(alpha);

                    if (!(value >= 0 && value <= 1.0f)) {
                        std::string msg = "The provided value (" + std::to_string(value) +
                                          ") is outside the range (0.0, 1.0).";
                    }

                    value = std::max(0.0f, std::min(value, 1.0f));
                    c.components[component] = value * 255.0f;
                    component++;
                } else if (isdigit(color[i])) {
                    c.components[component] = c.components[component] * 10 + (color[i] - '0');
                } else if (color[i] == ',' || color[i] == ')') {
                    component++;
                }
            }

            // set a default value for alpha component if no input
            if (component == 2) {
                c.components[component + 1] = 255.0f;
            }
        }
        // hsl(120,100%,50%) or hsla(120,100%,50%,0.5) format
            ...
            for (int i = 4; i < len - 1 && component < 4; i++) {
                if (component == 3) {
                    ...
                    hsla[component] = std::max(0.0f, std::min(value, 1.0f));
                    component++;
            ...
            c = HSLAtoColor(hsla[0] / 360.0f, hsla[1] / 100.0f, hsla[2] / 100.0f, hsla[3]);
        }  else {
            unsigned int hash = ColorHashForString(colorStr.c_str(), colorStr.length());
            c = ColorNames[ColorHashesToIndex[hash]];
        }
        return toSkColor(c);
    }
